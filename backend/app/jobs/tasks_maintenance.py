...
def close_days() -> dict:
    ...
    with SessionLocal() as db:
        for act in db.execute(
            select(Activity).where(Activity.status.in_(["active", "paused"]))
        ).scalars():
            try:
...
def cleanup() -> dict:
    ...
    with SessionLocal() as db:
        for s in db.execute(
            select(AuthSession).where(AuthSession.expires_at < now - timedelta(days=7))
        ).scalars():
            db.delete(s)
            removed["auth_sessions"] += 1
        for t in db.execute(
            select(OneTimeToken).where(OneTimeToken.expires_at < now - timedelta(days=7))
        ).scalars():
            db.delete(t)
            removed["tokens"] += 1
        for o in db.execute(
