    ...
    <header className="sticky top-0 z-30 border-b border-divider bg-[color-mix(in_srgb,var(--color-bg-canvas)_82%,transparent)] pt-[env(safe-area-inset-top,0px)] backdrop-blur-md tablet:hidden">
      <div className="flex h-14 items-center justify-between gap-3 px-gutter max-xs:px-3">
        <Link
          ...
          aria-label={t("Estudatta — ir para Hoje")}
        >
          <Symbol size={26} />
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/app/notificacoes"
            className={round}
            aria-label={unread > 0 ? t("Notificações: {{v0}} não lidas", { v0: unread }) : t("Notificações")}
          ...
          >
            ...
            {unread > 0 ? (
              <span
                aria-hidden
                className="absolute -right-1 -top-1 min-w-[18px] rounded-full bg-pending px-1 text-center text-[10px] font-semibold leading-[18px] text-on-accent"
              >
                {unread > 9 ? "9+" : unread}
              </span>
            ) : null}
