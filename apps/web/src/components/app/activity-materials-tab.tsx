...
export function ActivityMaterialsTab({
  ...
  const open = async (m: Material) => {
    ...
    try {
      ...
      if (!target) {
        tab?.close();
        toast.info(
          tx("Este material não tem arquivo para abrir"),
          m.kind === "physical" ? tx("É um material físico: use as páginas como referência.") : undefined,
        );
        return;
      }
      ...
      tab?.close();
      toast.error(tx("Não foi possível abrir o material"), errorMessage(err));
    } finally {
      setOpening(null);
    }
  };
  ...
  const shortcuts = (
    <div className="flex flex-wrap gap-2">
      <Button asChild variant="secondary" size="lg" className="flex-1">
        <Link to={`/app/materiais?objetivo=${activityId}`}>{tx("Gerenciar materiais")}</Link>
      </Button>
      <Button asChild variant="secondary" size="lg" className="flex-1">
        <Link to={`/app/importar?objetivo=${activityId}`}>{tx("Importar edital ou plano")}</Link>
      </Button>
    </div>
  );
  ...
  if (materials.isError) {
    return (
      ...
      >
        {online
          ? tx("Não foi possível carregar os materiais.")
          : tx("Sem conexão: os materiais aparecem quando você voltar à internet.")}
      </Banner>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-[6px] text-[14px]">
      ...
      {materials.data.length === 0 ? (
        <EmptyState
          title={tx("Nenhum material neste objetivo.")}
          description={tx("Adicione PDFs, links ou livros e vincule cada um ao tópico, com as páginas.")}
        />
      ) : (
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="flex min-w-0 flex-col">
                ...
                <span className="tnum text-[12px] text-neutral-400">
                  {[
                    ...
                    m.pages_total ? tx("{{v0}} páginas", { v0: m.pages_total }) : null,
                    m.last_position ? tx("parou em {{v0}}", { v0: m.last_position }) : null,
                  ]
                    .filter(Boolean)
                    .join(" · ")}
                </span>
              ...
              {m.kind !== "physical" ? (
                <Button
                  ...
                  onClick={() => void open(m)}
                >
                  {tx("Abrir")} <ArrowSquareOut size={14} aria-hidden />
                </Button>
              ) : null}
            </div>
            {(m.topics ?? []).length > 0 ? (
                  ...
                  <div key={t.topic_id} className="flex items-center justify-between gap-2">
                    ...
                    <span className="flex shrink-0 items-center gap-1">
                      ...
                      <button
                        ...
                        aria-label={tx("Desvincular {{v0}} de {{v1}}", {
                          v0: m.title,
                          v1: t.topic_title ?? tx("tópico"),
                        })}
                        disabled={unlink.isPending}
                        onClick={async () => {
                          try {
                            await unlink.mutateAsync({ materialId: m.id, topicId: t.topic_id });
                            toast.success(tx("Vínculo removido"));
                          } catch (err) {
                            toast.error(tx("Não foi possível desvincular"), errorMessage(err));
                          }
                        }}
                      >
              ...
              <span className="pl-3 text-[12px] text-neutral-400">{tx("sem tópico vinculado")}</span>
            )}
            <Button variant="ghost" size="sm" className="self-start pl-3" onClick={() => setLinkFor(m)}>
              {tx("+ Vincular a tópico")}
            </Button>
          </Card>
        ))
      )}
...
function LinkTopicSheet({
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    if (!topicId) return setError(tx("Escolha o tópico."));
    const [from, to] = parsePages(pages);
    if (from != null && to != null && to < from)
      return setError(tx("A página final precisa ser maior que a inicial."));
    try {
      await link.mutateAsync({
        materialId: material.id,
        body: { topic_id: topicId, page_from: from, page_to: to, note: note.trim() || null },
      });
      toast.success(tx("Material vinculado ao tópico"));
      onOpenChange(false);
    } catch (err) {
  ...
  return (
    <Dialog open onOpenChange={onOpenChange}>
      <DialogContent mode="sheet" title={tx("Vincular a tópico")} description={material.title}>
        <form onSubmit={submit} className="flex flex-col gap-[14px]">
          ...
          {subjects.isPending ? (
            ...
            <Banner kind="info">
              {tx(
                "Este objetivo ainda não tem tópicos. Crie matérias e tópicos na aba Matérias para vincular materiais.",
              )}
            </Banner>
          ) : (
            <>
              <Field label={tx("Tópico")} htmlFor="lk-topic">
                <Select id="lk-topic" value={topicId} onChange={(e) => setTopicId(e.target.value)}>
                  <option value="">{tx("— escolher —")}</option>
                  {groups.map((g) => (
              ...
              <Field label={tx("Páginas (opcional)")} htmlFor="lk-pages">
                <Input
                  id="lk-pages"
                  placeholder={tx("p. 12–34")}
                  value={pages}
                  onChange={(e) => setPages(e.target.value)}
                />
              </Field>
              <Field label={tx("Observação (opcional)")} htmlFor="lk-note">
                <Input
                  id="lk-note"
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  maxLength={500}
                  placeholder={tx("Capítulo 3, exercícios ímpares")}
                />
              </Field>
