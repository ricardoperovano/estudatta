...
const REASON: Record<string, string> = {
  nao_coube: t("não coube na disponibilidade da semana"),
  sem_estimativa: t("sem duração estimada"),
};
...
export function AutoPlanDialog({ open, onOpenChange, activities, defaultActivityId, start, end }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        title={t("Distribuir tarefas na semana")}
        description={t(
          "{{v0}} · compare o plano antes de aplicar. Tarefas fixadas, séries e concluídas não saem do lugar.",
          { v0: fmtRange(start, end) },
        )}
        width="min(560px, calc(100% - 32px))"
      >
        {activities.length > 1 ? (
          <Field label={t("Objetivo")} htmlFor="ap-activity">
        ...
        {activityId ? (
          ...
          <Banner kind="info">{t("Crie um objetivo para distribuir tarefas.")}</Banner>
        )}
      </DialogContent>
    </Dialog>
  );
...
function AutoPlanBody({
  ...
  const column = (kind: "before" | "after") => (
    <Card className={cn("gap-1.5 bg-canvas p-[14px]", kind === "after" && "shadow-accent-ring")}>
      <span className={kind === "after" ? "kicker-accent" : "kicker"}>
        {kind === "after" ? t("Depois") : t("Antes")}
      </span>
      {(data?.days ?? []).map((d) => {
        ...
        return (
          <span
            key={d.local_date}
            className={cn("flex flex-col", !d.is_active && titles.length === 0 && "text-neutral-500")}
          >
            <span className="flex justify-between gap-2">
              <span>{fmtDayTiny(d.local_date)}</span>
              <span className={cn(over && "text-pending")}>
                {seconds + d.fixed_seconds > 0 ? minutesOf(seconds + d.fixed_seconds) : "—"}
              </span>
            </span>
            {titles.length > 0 ? (
              <span className="truncate text-[12px] text-neutral-400">{titles.join(" · ")}</span>
            ) : null}
          </span>
        );
      })}
    ...
    <>
      {preview.isPending || preview.isIdle ? (
        ...
        <Banner
          kind="error"
          actions={
            <Button size="sm" variant="secondary" onClick={() => run({ start, end })}>
              {t("Tentar de novo")}
            </Button>
          }
        >
        ...
        <>
          {nothing ? (
            <Banner kind="info">
              {t("Nada a mover: as tarefas de estudo desta semana já cabem nos dias em que estão.")}
            </Banner>
          ) : null}
          ...
          {data && data.moves.length > 0 ? (
            <div className="flex flex-col gap-1 text-[13px]">
              <span className="kicker">{t("O que muda")}</span>
              {data.moves.map((m) => (
          ...
          {data && data.unplaced.length > 0 ? (
            <Banner kind="info">
              {data.unplaced.length === 1
                ? t("1 tarefa fica onde está")
                : t("{{v0}} tarefas ficam onde estão", { v0: data.unplaced.length })}
              : {data.unplaced.map((u) => `${u.title} (${REASON[u.reason] ?? u.reason})`).join("; ")}.
            </Banner>
          ) : null}
        </>
      )}
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                toast.success(
                  t("Plano da semana atualizado"),
                  t("{{v0}} {{v1}} de dia.", {
                    v0: out.moves.length,
                    v1: out.moves.length === 1 ? t("tarefa mudou") : t("tarefas mudaram"),
                  }),
                );
                onClose();
              } catch {
