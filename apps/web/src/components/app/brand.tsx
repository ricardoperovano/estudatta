...
export function Symbol({ size = 24, className }: { size?: number; className?: string }) {
  ...
  return (
    <span className={cn("brand-logo inline-block shrink-0", className)} style={{ width: box, height: box }} aria-hidden>
      <img src={brand.logos.escuro} alt="" width={box} height={box} className="brand-logo-dark h-full w-full rounded-[22%]" />
      <img src={brand.logos.claro} alt="" width={box} height={box} className="brand-logo-light h-full w-full rounded-[22%]" />
    </span>
  );
}
