...
export const EMPTY_STUDY_FIELDS: StudyFieldsValue = { study_type: "teoria", questions_total: null, questions_correct: null };
...
export function studyFieldsFrom(s: { study_type?: string | null; questions_total?: number | null; questions_correct?: number | null }): StudyFieldsValue {
  const t = STUDY_TYPES.some((x) => x.value === s.study_type) ? (s.study_type as StudyType) : "teoria";
  return { study_type: t, questions_total: s.questions_total ?? null, questions_correct: s.questions_correct ?? null };
}
...
export function validateStudyFields(v: StudyFieldsValue): string | null {
  ...
  if ((total != null && (!Number.isInteger(total) || total < 0)) || (correct != null && (!Number.isInteger(correct) || correct < 0))) {
    return "Use números inteiros a partir de 0 para questões e acertos.";
  }
  if (total != null && total > MAX_QUESTIONS) return `O limite é de ${MAX_QUESTIONS} questões por sessão.`;
  if (correct != null && total == null) return "Informe quantas questões você fez.";
  // o servidor grava acertos em branco como 0; melhor pedir do que mostrar 0% sem querer
  if (total != null && correct == null) return "Informe também quantas você acertou.";
  if (correct != null && total != null && correct > total) return "Os acertos não podem passar do número de questões feitas.";
  return null;
}
...
export function studyFieldsPayload(v: StudyFieldsValue): StudyFieldsValue & { hasQuestions: boolean } {
  ...
  return { study_type: v.study_type, questions_total: total, questions_correct: correct, hasQuestions: total != null };
}
...
export function StudyFields({ value, onChange, compact, idPrefix = "study", className }: StudyFieldsProps) {
  ...
  const error = value.questions_total != null && value.questions_correct != null ? validateStudyFields(value) : null;
  ...
  return (
    <div className={cn("flex flex-col gap-[14px]", className)}>
      {compact ? (
        <Field label="Tipo de estudo" htmlFor={`${idPrefix}-type`}>
          <Select id={`${idPrefix}-type`} value={value.study_type} onChange={(e) => setType(e.target.value as StudyType)}>
            {STUDY_TYPES.map((t) => (
        ...
        <div className="flex flex-col">
          <span id={`${idPrefix}-type-label`} className="mb-[5px] block text-[12px] text-[color-mix(in_srgb,var(--color-text-primary)_70%,transparent)]">
            Tipo de estudo
          </span>
          <div role="radiogroup" aria-labelledby={`${idPrefix}-type-label`} className="grid grid-cols-4 gap-1.5">
            {STUDY_TYPES.map((t) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "inline-flex min-h-[36px] min-w-0 items-center justify-center rounded-md border border-divider px-1.5 py-[6px] text-[13px] transition-colors duration-base cursor-pointer",
      ...
      {ask ? (
        <div className="grid grid-cols-2 gap-2">
          <Field label="Questões feitas" htmlFor={`${idPrefix}-q-total`}>
            <Input
              id={`${idPrefix}-q-total`}
              type="number"
              inputMode="numeric"
          ...
          <Field label="Acertos" htmlFor={`${idPrefix}-q-correct`} error={error ?? undefined}>
            <Input
...
export function fmtQuestions(total: number | null | undefined, correct: number | null | undefined): string | null {
  if (total == null || total <= 0) return null;
  if (correct == null) return `${total} ${total === 1 ? "questão" : "questões"}`;
  return `${correct}/${total} questões (${Math.round((correct / total) * 100)}%)`;
}
