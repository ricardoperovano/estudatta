...
const TITLE: Record<SettingKey, string> = {
  brand: t("Marca e contato"),
  feature_flags: t("Recursos ligados"),
  limits: t("Limites gerais"),
};
const HELP: Record<SettingKey, string> = {
  brand: t("Dados públicos da marca. Campos vazios ficam como pendentes."),
  feature_flags: t("Liga ou desliga recursos. O padrão vem das credenciais configuradas no servidor."),
  limits: t("Limites aplicados de forma geral, no mesmo formato dos limites de plano."),
};

const BRAND_FIELDS: { key: string; label: string; type?: string }[] = [
  { key: "name", label: t("Nome da marca") },
  { key: "support_email", label: t("E-mail de suporte"), type: "email" },
  { key: "company_name", label: t("Nome fantasia da empresa") },
  { key: "legal_name", label: t("Razão social") },
  { key: "cnpj", label: "CNPJ" },
  { key: "phone", label: t("Telefone"), type: "tel" },
  { key: "address", label: t("Endereço") },
];
  ...
  return (
    <div className="flex flex-col gap-[14px] desktop:gap-6">
      <AdminTitle
        ...
        subtitle={t("Cada bloco é salvo separadamente e fica registrado na auditoria.")}
      />
      {SETTING_KEYS.map((k) => (
  ...
  return (
    <AdminSection
      ...
      meta={
        s ? (
          s.is_default ? (
            <Tag variant="neutral">{t("padrão")}</Tag>
          ) : (
            <span className="tnum">
              {t("salvo {{v0}} por {{v1}}", { v0: fmtDate(s.updated_at), v1: shortId(s.updated_by) })}
            </span>
          )
        ) : null
      }
    >
      ...
      <QueryGate query={setting}>
        {(data) => (
          <SettingEditor
            key={`${data.updated_at ?? "default"}:${stableJson(data.value)}`}
            setting={data}
...
function SettingEditor({ setting, settingKey }: { setting: SettingOut; settingKey: SettingKey }) {
  ...
  if (mode === "json") {
    try {
      ...
      else jsonError = t("O valor precisa ser um objeto JSON ({ … }).");
    } catch (e) {
      jsonError = t("JSON inválido: {{v0}}", { v0: e instanceof Error ? e.message : t("erro de sintaxe") });
    }
  }
  ...
  const switchMode = (m: "form" | "json") => {
    ...
    if (m === "json") {
      ...
      toast("error", t("Corrija o JSON antes de voltar ao formulário"));
    }
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast("success", t("Configuração salva"), TITLE[settingKey]);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <Seg
        label={t("Modo de edição")}
        value={mode}
        onChange={switchMode}
        options={[
          { value: "form", label: t("Formulário") },
      ...
      {mode === "json" ? (
        <Field label={t("Valor (JSON)")} htmlFor={`setting-json-${settingKey}`} error={jsonError}>
          <Textarea
            id={`setting-json-${settingKey}`}
            value={json}
...
function FlagsForm({ value, onChange }: { value: Value; onChange: (v: Value) => void }) {
  ...
  const nameError = !name
    ? null
    : !FLAG_RE.test(name)
      ? t("Minúsculas, números e _, começando por letra (até 32).")
      : name in value
        ? t("Já existe.")
        : null;
    ...
    <div className="flex flex-col gap-1">
      {Object.entries(value).map(([k, v]) =>
        typeof v === "boolean" ? (
          <label
            key={k}
            className="flex min-h-[44px] items-center justify-between gap-3 border-b border-divider text-[14px]"
          >
            <span>
          ...
          <div key={k} className="rounded-md bg-error-tint p-2 text-[13px]">
            <code>{k}</code> {t("não é verdadeiro/falso; corrija pelo modo JSON.")}
          </div>
        ),
      )}
      <form onSubmit={add} className="mt-2 flex flex-wrap items-start gap-2" noValidate>
        <Field error={nameError} className="min-w-0 flex-1 tablet:max-w-[280px]">
          <Input
            aria-label={t("Nome do novo recurso")}
            placeholder="novo_recurso"
            value={name}
...
function BrandForm({ value, onChange }: { value: Value; onChange: (v: Value) => void }) {
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        {BRAND_FIELDS.map((f) => (
          <Field key={f.key} label={f.label} htmlFor={`brand-${f.key}`}>
            <Input
              ...
              type={f.type ?? "text"}
              maxLength={300}
              value={text(value[f.key])}
              onChange={(e) => onChange({ ...value, [f.key]: e.target.value || null })}
            />
          </Field>
        ))}
      </div>
      <span className="kicker">{t("Redes sociais")}</span>
      <div className="grid grid-cols-1 gap-3 tablet:grid-cols-2">
        {SOCIAL_KEYS.map((k) => (
          <Field key={k} label={k} htmlFor={`brand-social-${k}`}>
            <Input
              id={`brand-social-${k}`}
              maxLength={300}
              placeholder={t("Endereço ou usuário")}
              value={text(social[k])}
      ...
      {extra.length ? (
        <Banner kind="info">
          {t("Campos fora do formulário ({{v0}}): edite pelo modo JSON.", { v0: extra.join(", ") })}
        </Banner>
      ) : null}
    </div>
  );
