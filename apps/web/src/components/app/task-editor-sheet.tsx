...
export function TaskEditorSheet({ open, onOpenChange, activities, task, defaults }: Props) {
  ...
  const [activityId, setActivityId] = React.useState(
    task?.activity_id ?? defaults?.activityId ?? activities[0]?.id ?? "",
  );
  const [title, setTitle] = React.useState(task?.title ?? "");
  const [kind, setKind] = React.useState<"study" | "checklist">(
    (task?.kind as "study" | "checklist") ?? defaults?.kind ?? "study",
  );
  ...
  const initialMinutes = task?.estimated_seconds
    ? String(Math.round(task.estimated_seconds / 60))
    : kind === "study"
      ? "30"
      : "";
  const [preset, setPreset] = React.useState(
    PRESETS.includes(initialMinutes) ? initialMinutes : initialMinutes ? "outro" : "30",
  );
  ...
  const [pages, setPages] = React.useState(
    fmtPages(task?.page_from, task?.page_to)?.replace("p. ", "") ?? "",
  );
  const [notes, setNotes] = React.useState(task?.notes ?? "");
  const [repeat, setRepeat] = React.useState(false);
  const [weekdays, setWeekdays] = React.useState<number[]>([
    weekdayMon(task?.local_date ?? defaults?.date ?? todayIso()),
  ]);
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    if (!activityId) return setError(tx("Escolha um objetivo."));
    if (!title.trim()) return setError(tx("Dê um nome à tarefa."));
    if (kind === "study" && minutes < 5) return setError(tx("Sessões precisam de pelo menos 5 minutos."));
    const [pf, pt] = parsePages(pages);
    try {
      if (!editing) {
        if (repeat) {
          if (weekdays.length === 0) return setError(tx("Escolha ao menos um dia da semana."));
          ...
          toast.success(tx("Série criada"), tx("As ocorrências aparecem no plano da semana."));
        } else {
          ...
          toast.success(kind === "study" ? tx("Sessão planejada") : tx("Tarefa adicionada"));
        }
        close();
        return;
      }
      ...
      if (t.virtual && t.series_id) {
        ...
        if (dateChanged && real.id && date !== t.local_date) {
          await reschedule.mutateAsync({
            id: real.id,
            body: { local_date: date, start_time: time || null, clear_start_time: !time },
          });
        }
      } else if (t.id) {
      ...
      toast.success(
        dateChanged ? tx("Tarefa reagendada") : tx("Tarefa atualizada"),
        dateChanged ? tx("É a mesma tarefa, só mudou de lugar.") : undefined,
      );
      close();
    } catch (err) {
    ...
    try {
      if (t.virtual && t.series_id) {
        await materialize.mutateAsync({
          seriesId: t.series_id,
          date: t.local_date,
          body: { status: "skipped", clear_start_time: false },
        });
      } else if (t.id) {
        await deleteTask.mutateAsync(t.id);
      }
      toast.success(tx("Tarefa removida"));
      setConfirm(null);
      close();
    } catch (err) {
      setError(errorMessage(err));
  ...
  const doSkip = async () => {
    ...
    try {
      ...
      toast.info(tx("Tarefa pulada"), tx("Ela não conta como concluída nem como pendência."));
      close();
    } catch (err) {
      setError(errorMessage(err));
    }
  ...
  const doEndSeries = async () => {
    try {
      await updateSeries.mutateAsync({
        id: task!.series_id!,
        body: { active: false, clear_end_date: false, clear_start_time: false },
      });
      toast.success(tx("Série encerrada"), tx("As ocorrências futuras deixam de aparecer."));
      setConfirm(null);
      close();
    } catch (err) {
      setError(errorMessage(err));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        mode="sheet"
        title={editing ? tx("Editar tarefa") : tx("Nova tarefa")}
        description={
          editing
            ? tx("Mudar data ou horário reagenda a mesma tarefa.")
            : tx("Uma sessão ocupa a meta do dia; uma tarefa é só um item para marcar.")
        }
      >
        <form onSubmit={submit} className="flex flex-col gap-[14px]">
          {error ? <Banner kind="error">{error}</Banner> : null}
          {!editing && activities.length > 1 ? (
            <Field label={tx("Objetivo")} htmlFor="t-activity">
              <Select
                id="t-activity"
                value={activityId}
                onChange={(e) => {
          ...
          <Field label={tx("Título")} htmlFor="t-title">
            <Input
              id="t-title"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder={kind === "study" ? tx("Listening · unidade 4") : tx("Revisar unidade 4")}
              maxLength={200}
              autoFocus={!editing}
            />
          </Field>
          <Seg
            ...
            options={[
              { value: "study", label: tx("Sessão de estudo") },
              { value: "checklist", label: tx("Tarefa") },
            ]}
          />
          <div className="grid grid-cols-2 gap-2">
            <Field label={tx("Data")} htmlFor="t-date">
              <Input id="t-date" type="date" value={date} onChange={(e) => setDate(e.target.value)} />
            </Field>
            <Field label={tx("Horário (opcional)")} htmlFor="t-time">
              <Input id="t-time" type="time" value={time} onChange={(e) => setTime(e.target.value)} />
            </Field>
          </div>
          <Field label={kind === "study" ? tx("Duração estimada") : tx("Duração estimada (opcional)")}>
            <Seg
              block
              label={tx("Duração")}
              value={preset}
            ...
            {preset === "outro" ? (
              <Input
                ...
                min={0}
                max={600}
                placeholder={tx("Minutos")}
                value={customMin}
                onChange={(e) => setCustomMin(e.target.value)}
                aria-label={tx("Minutos")}
              />
            ) : null}
          </Field>
          {subjects.data && subjects.data.length > 0 ? (
            <div className="grid grid-cols-2 gap-2">
              <Field label={tx("Matéria (opcional)")} htmlFor="t-subject">
                <Select
                  id="t-subject"
                  value={subjectId}
                  onChange={(e) => {
              ...
              <Field label={tx("Tópico (opcional)")} htmlFor="t-topic">
                <Select
                  id="t-topic"
                  value={topicId}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label={tx("Páginas (opcional)")} htmlFor="t-pages">
              <Input
                id="t-pages"
                placeholder="40–46"
                value={pages}
            ...
            <Field label={tx("Observações (opcional)")} htmlFor="t-notes">
              <Textarea
                id="t-notes"
            ...
            <div className="flex flex-col gap-3 rounded-md bg-canvas p-3">
              <label className="flex items-center justify-between gap-3 text-[14px]">
                <span className="flex items-center gap-2">
                  <Repeat size={16} aria-hidden /> {tx("Repetir toda semana")}
                </span>
                <Switch checked={repeat} onCheckedChange={setRepeat} label={tx("Repetir toda semana")} />
              </label>
              {repeat ? (
                <>
                  <DayPicker value={weekdays} onChange={setWeekdays} />
                  <Field label={tx("Até (opcional)")} htmlFor="t-until">
                    <Input
                      id="t-until"
                      type="date"
                      min={date}
          ...
          {editing && task?.series_id ? (
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-canvas p-3 text-[13px] text-neutral-400">
              <span className="flex items-center gap-2">
                <Repeat size={14} aria-hidden /> {tx("Parte de uma série semanal")}
              </span>
      ...
      <ConfirmDialog
        ...
        title={tx("Excluir esta tarefa?")}
        description={tx("Ela sai do plano. Tempo já registrado não muda.")}
      ...
      <ConfirmDialog
        ...
        onOpenChange={(o) => !o && setConfirm(null)}
        title={tx("Encerrar a série?")}
        description={tx(
          "As ocorrências futuras deixam de aparecer. As já concluídas continuam no histórico.",
        )}
        confirmLabel={tx("Encerrar")}
      ...
      <ConfirmDialog
        ...
        onOpenChange={(o) => !o && setConfirm(null)}
        title={tx("Excluir a série inteira?")}
        description={tx("Todas as ocorrências planejadas são removidas. Tempo já registrado não muda.")}
