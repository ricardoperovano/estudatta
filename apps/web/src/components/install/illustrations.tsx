...
export function Mock({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      aria-hidden
      className={cn(
        "pointer-events-none select-none rounded-lg border border-divider bg-canvas p-2 text-primary",
        className,
      )}
    >
      {children}
...
function Hi({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={cn(
        "relative inline-flex items-center justify-center rounded-md bg-accent-tint text-accent ring-2 ring-accent",
        className,
      )}
    >
...
function Dim({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <span className={cn("inline-flex items-center justify-center text-neutral-500", className)}>
      {children}
    </span>
  );
}
  ...
  return (
    <Mock>
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <Hi className="h-8 w-8">
          <Export size={IC} weight="bold" />
        </Hi>
        <Dim>
          <BookOpen size={IC} />
        </Dim>
    ...
    <Mock className="p-1.5">
      <ul className="flex flex-col gap-1 text-[12px]">
        {rows.map(([label, icon]) => (
          <li
            key={label}
            className="flex items-center justify-between rounded-md px-2 py-1.5 text-neutral-500"
          >
  ...
  return (
    <Mock>
      <div className="flex items-center justify-between gap-2 text-[11px]">
        <span className="text-neutral-500">{t("Cancelar")}</span>
        <span className="truncate font-medium">{t("Tela de Início")}</span>
        <Hi className="px-2 py-1 text-[12px] font-semibold">{t("Adicionar")}</Hi>
      </div>
      ...
      <div className="mt-1.5 flex items-center justify-between rounded-md bg-surface px-2 py-1 text-[11px] text-neutral-500">
        <span>{t("Abrir como App Web")}</span>
  ...
  return (
    <Mock>
      ...
      <div className="mt-2 flex justify-end gap-2 text-[12px]">
        <span className="px-2 py-1 text-neutral-500">{t("Cancelar")}</span>
  ...
  return (
    <Mock className="p-1.5">
      <div className="mb-1 flex gap-3 px-1.5 text-[11px] text-neutral-500">
        <span className="font-semibold text-primary">{t("Safari")}</span>
        <span className="rounded-sm bg-accent-tint px-1 text-accent">{t("Arquivo")}</span>
        <span>{t("Editar")}</span>
        <span>{t("Visualizar")}</span>
      </div>
      <MenuRowsInline rows={["Nova Janela", "Nova Aba"]} highlight={t("Adicionar ao Dock…")} />
    </Mock>
  );
}
...
function MenuRowsInline({ rows, highlight }: { rows: string[]; highlight: string }) {
  return (
    <ul className="flex flex-col gap-0.5 text-[12px]">
      ...
      <li className="rounded-md bg-accent-tint px-2 py-1 font-medium text-accent ring-2 ring-accent">
        {highlight}
      </li>
    </ul>
  );
