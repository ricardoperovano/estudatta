...
function useFirstSteps(hasObjective: boolean) {
  ...
  const hasSession =
    !!g.data &&
    (g.data.records.total_seconds > 0 ||
      g.data.achievements.some((a) => a.code === "first_session" && a.unlocked));
  const steps: Step[] = [
    {
      key: "objetivo",
      title: t("Criar seu primeiro objetivo"),
      hint: t("Concurso, idioma, instrumento… com dias e meta diária."),
    ...
    {
      key: "sessao",
      title: t("Fazer a primeira sessão"),
      hint: t("Ligue o cronômetro ou registre um tempo que já estudou."),
      to: "/app/sessao",
      icon: Timer,
      done: hasSession,
    },
    {
      key: "lembretes",
      title: t("Ativar lembretes"),
      hint: t("Um toque gentil no horário que você escolher."),
      to: "/app/preferencias",
      icon: BellRinging,
    ...
    {
      key: "instalar",
      title: t("Instalar o app no celular"),
      hint: t("Abre direto da tela inicial, como um app."),
      to: "/app/instalar",
      icon: DeviceMobile,
      done: isStandalone(),
    },
    {
      key: "conquistas",
      title: t("Conhecer suas conquistas"),
      hint: t("Níveis e medalhas que não se perdem se a sequência quebrar."),
      to: "/app/conquistas",
      icon: Trophy,
      done: achSeen,
      onClick: () => {
...
export function FirstSteps({ hasObjective, className }: { hasObjective: boolean; className?: string }) {
  ...
  return (
    <section
      ...
      className={cn(
        "tint-soft rise-in flex flex-col gap-3 rounded-[20px] p-4 shadow-sm desktop:p-5",
        className,
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <span className="kicker-accent">{t("Primeiros passos")}</span>
          <h2 id="primeiros-passos" className="mt-1 text-[18px] leading-[1.2]">
            {hasObjective ? t("Falta pouco para deixar tudo pronto") : t("Vamos deixar tudo pronto?")}
          </h2>
        </div>
        {hasObjective ? (
          <Button
            ...
            aria-label={t("Dispensar primeiros passos")}
            title={t("Dispensar")}
          >
      ...
      <div className="flex items-center gap-3">
        <div
          ...
          role="img"
          aria-label={t("{{v0}} de {{v1}} passos feitos", { v0: done, v1: steps.length })}
        >
          {steps.map((s) => (
            <span
              key={s.key}
      ...
      {hasObjective ? (
        <button
          ...
          className="flex items-center gap-1 self-start rounded-md px-1 py-1 text-[13px] text-accent hover:underline"
        >
          {expanded ? t("Mostrar só o próximo") : t("Ver os {{v0}} passos", { v0: steps.length })}
          <CaretDown
            size={12}
            aria-hidden
    ...
    >
      ...
      >
        {step.done ? (
          ...
          <I size={18} weight={highlight ? "fill" : "regular"} />
        )}
      </span>
      <span className="min-w-0 flex-1 leading-tight">
        <span className={cn("block text-[14px] font-medium", step.done && "text-neutral-400")}>
          {step.title}
          {step.done ? <span className="sr-only"> {t("(feito)")}</span> : null}
        </span>
      ...
      {!step.done ? (
        <CaretRight
          size={14}
          className={cn("shrink-0", highlight ? "text-accent" : "text-neutral-500")}
          aria-hidden
        />
      ) : null}
    </Link>
