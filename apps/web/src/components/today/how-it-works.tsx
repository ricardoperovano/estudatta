...
const STEPS: { icon: Icon; title: string; body: string; tint: string }[] = [
  {
    icon: Target,
    title: t("Defina a meta"),
    body: t("Escolha os dias e quantos minutos por dia. Pode ser pouquinho."),
    tint: "bg-accent-900 text-accent",
  },
  {
    icon: Timer,
    title: t("Estude com o Tatá"),
    body: t("Use o cronômetro ou registre depois o tempo que já estudou."),
    tint: "bg-info-tint text-info",
  },
  {
    icon: ArrowsClockwise,
    title: t("Siga no seu ritmo"),
    body: t("Atrasou? O plano mostra como recuperar aos poucos, sem culpa."),
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <li key={s.title} className="flex items-start gap-3">
              <span
                className={cn("relative grid h-10 w-10 shrink-0 place-items-center rounded-[12px]", s.tint)}
                aria-hidden
              >
