...
function GlyphBubble({ children, large }: { children: React.ReactNode; large?: boolean }) {
  return (
    <span className={cn("relative grid shrink-0 place-items-center rounded-full bg-accent-900", large ? "h-[104px] w-[104px]" : "h-[72px] w-[72px]")} aria-hidden>
      <span className="absolute -right-1 top-1 h-2.5 w-2.5 rounded-full bg-warning-tint shadow-[inset_0_0_0_1.5px_var(--color-status-warning)] opacity-80" />
      <span className="absolute -left-1.5 bottom-3 h-2 w-2 rounded-full bg-info-tint shadow-[inset_0_0_0_1.5px_var(--color-status-info)] opacity-80" />
      <svg viewBox="0 0 16 16" className="absolute -left-0.5 top-0 h-3 w-3 text-accent opacity-70" fill="currentColor">
 ...
 */
export function EmptyState({ title, description, action, className, glyph, mascot, variant = "plain" }: EmptyStateProps) {
  const card = variant === "card";
  return (
    <div className={cn("flex flex-col items-center gap-2 px-5 py-6 text-center", card && "tint-soft rounded-[20px] px-6 py-8 shadow-sm", className)}>
      {mascot ? (
        <TataSvg mood={mascot} size={card ? 96 : 80} className="mb-1" />
      ) : (
        <GlyphBubble large={card}>{glyph ?? <TrailGlyph size={card ? 44 : 34} className="text-accent-500" />}</GlyphBubble>
      )}
      <span className={cn("mt-1 max-w-[36ch] font-medium text-primary", card ? "text-[17px]" : "text-[15px]")}>{title}</span>
      {description ? <span className="max-w-[44ch] text-[13px] leading-[1.5] text-neutral-400">{description}</span> : null}
