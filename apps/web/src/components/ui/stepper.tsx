    ...
    <div className={cn("flex flex-col items-center gap-3", className)}>
      <div className="flex items-center justify-center gap-3">
        <Button
          ...
          variant="secondary"
          size="icon-lg"
          aria-label={t("Menos {{v0}} minutos", { v0: step })}
        ...
        <span
          className={cn(
            "tnum min-w-[120px] text-center font-medium",
            size === "lg" ? "min-w-[150px] text-[42px]" : "text-[32px]",
          )}
          aria-live="polite"
        >
          {t("{{v0}} min", { v0: minutes })}
        ...
        <Button
          ...
          variant="secondary"
          size="icon-lg"
          aria-label={t("Mais {{v0}} minutos", { v0: step })}
          onClick={() => onChange(clamp(minutes + step))}
        >
          <Plus size={20} />
        </Button>
...
export function DayPicker({ value, onChange, className }: DayPickerProps) {
  ...
  const names = ["segunda", t("terça"), "quarta", "quinta", "sexta", t("sábado"), "domingo"];
  return (
    <div role="group" aria-label={t("Dias ativos")} className={cn("flex justify-between gap-1.5", className)}>
      {labels.map((l, i) => {
        ...
        return (
          <button
            ...
            className={cn(
              "h-11 w-11 cursor-pointer rounded-md border text-[14px] font-medium transition-colors duration-base max-xs:w-10",
              active
                ? "border-accent bg-accent-900 text-accent"
                : "border-divider text-neutral-400 hover:bg-[color-mix(in_srgb,var(--color-text-primary)_7%,transparent)]",
            )}
          >
            {l}
          </button>
