...
export const NETWORK_ERROR_MESSAGE = "Sem conexão. Sua ação foi salva neste aparelho e será sincronizada.";
...
export function unwrap<T>(res: { data?: T; error?: unknown; response: Response }): T {
  if (res.error !== undefined || !res.response.ok) {
    ...
    throw new ApiError(
      res.response.status,
      err?.code || "http_error",
      err?.message || `Erro ${res.response.status}`,
      err?.details,
      err?.request_id,
    );
  }
...
export async function rawJson<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(res.status, body.error?.code || "http_error", body.error?.message || `Erro ${res.status}`, body.error?.details);
  }
