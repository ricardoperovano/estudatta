...
export const studyTypeLabel = (v: string | null | undefined) => STUDY_TYPES.find((t) => t.value === v)?.label ?? "Teoria";
...
// ---------------------------------------------------------------- revisões
export function useRevisions(params: { status?: "pending" | "done" | "skipped"; activity_id?: string; until?: string } = {}) {
  const user = useUser();
  return useQuery({
    queryKey: studyKeys.revisions(params),
    enabled: !!user,
    queryFn: async () => unwrap(await api.GET("/api/v1/revisions", { params: { query: params } })) as Revision[],
    staleTime: 30_000,
  });
}
...
export function useRevisionAction() {
  ...
  return useMutation({
    mutationFn: async ({ id, action, due_date }: { id: string; action: "done" | "skip" | "reschedule"; due_date?: string }) => {
      const path = { revision_id: id };
      if (action === "done") return unwrap(await api.POST("/api/v1/revisions/{revision_id}/done", { params: { path } }));
      if (action === "skip") return unwrap(await api.POST("/api/v1/revisions/{revision_id}/skip", { params: { path } }));
      return unwrap(await api.POST("/api/v1/revisions/{revision_id}/reschedule", { params: { path }, body: { due_date: due_date! } }));
    },
...
export function useMockExams(activityId: string | undefined) {
  return useQuery({
    ...
    enabled: !!activityId,
    queryFn: async () => unwrap(await api.GET("/api/v1/activities/{activity_id}/mock-exams", { params: { path: { activity_id: activityId! } } })) as MockOverview,
  });
}
...
export function useSaveMockExam(activityId: string) {
  ...
  return useMutation({
    mutationFn: async ({ id, body }: { id?: string; body: MockExamIn }) =>
      id
        ? (unwrap(await api.PATCH("/api/v1/mock-exams/{exam_id}", { params: { path: { exam_id: id } }, body })) as MockExam)
        : (unwrap(await api.POST("/api/v1/activities/{activity_id}/mock-exams", { params: { path: { activity_id: activityId } }, body })) as MockExam),
    onSuccess: invalidate,
  });
}
...
export function useDeleteMockExam() {
  const invalidate = useInvalidateStudy();
  return useMutation({
    mutationFn: async (id: string) => unwrap(await api.DELETE("/api/v1/mock-exams/{exam_id}", { params: { path: { exam_id: id } } })),
    onSuccess: invalidate,
  });
}
...
export function useInsights(activityId: string | undefined) {
  return useQuery({
    ...
    enabled: !!activityId,
    queryFn: async () => unwrap(await api.GET("/api/v1/activities/{activity_id}/insights", { params: { path: { activity_id: activityId! } } })) as Insights,
    staleTime: 30_000,
  });
}
...
export function useMarkAchievementsSeen() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (codes?: string[]) => unwrap(await api.POST("/api/v1/gamification/seen", { body: { codes: codes ?? null } })),
