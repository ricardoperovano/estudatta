...
export function useUploadMaterial() {
  ...
  return useMutation({
    mutationFn: async ({ file, title, activityId }: { file: File; title?: string; activityId?: string | null }) => {
...
export function useUpdateMaterial() {
  ...
  return useMutation({
    mutationFn: async ({ id, body }: { id: string; body: MaterialUpdate }) =>
      unwrap(await api.PATCH("/api/v1/materials/{material_id}", { params: { path: { material_id: id } }, body })) as MaterialDetail,
    onSuccess: (d) => invalidate(d),
  });
}
...
export function useDeleteMaterial() {
  const invalidate = useInvalidateAllMaterials();
  return useMutation({
    mutationFn: async (id: string) => unwrap(await api.DELETE("/api/v1/materials/{material_id}", { params: { path: { material_id: id } } })),
...
export function viewerUrl(downloadUrl: string | null | undefined): string | null {
  ...
  try {
    ...
    if (!API_BASE && u.origin !== window.location.origin && u.pathname.startsWith("/api/")) return `${u.pathname}${u.search}`;
    return u.toString();
  } catch {
    return null;
  }
...
const KIND_LABEL: Record<string, string> = { pdf: "PDF", link: "link", physical: "físico" };
...
export function materialPagesLabel(m: Pick<Material, "page_from" | "page_to" | "pages_total">): string | null {
  if (m.page_from != null && m.page_to != null) return m.page_from === m.page_to ? `p. ${m.page_from}` : `p. ${m.page_from}–${m.page_to}`;
  if (m.page_from != null) return `a partir da p. ${m.page_from}`;
  if (m.pages_total != null) return `${m.pages_total} ${m.pages_total === 1 ? "página" : "páginas"}`;
  return null;
}
