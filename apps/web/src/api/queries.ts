...
export function useActivities(includeArchived = false) {
  return useQuery({
    ...
    queryFn: async () =>
      unwrap(
        await api.GET("/api/v1/activities", { params: { query: { include_archived: includeArchived } } }),
      ) as Activity[],
  });
}
...
export function useActivity(id: string | undefined) {
  return useQuery({
    ...
    queryFn: async () =>
      unwrap(
        await api.GET("/api/v1/activities/{activity_id}", { params: { path: { activity_id: id! } } }),
      ) as ActivityDetail,
  });
}
...
export function useBalance(id: string | undefined, days = 60) {
  return useQuery({
    ...
    queryFn: async () =>
      unwrap(
        await api.GET("/api/v1/activities/{activity_id}/balance", {
          params: { path: { activity_id: id! }, query: { days } },
        }),
      ) as Balance,
  });
}
...
export function useManualSession() {
  ...
  return useMutation({
    mutationFn: async (body: ManualBody) =>
      unwrap(await api.POST("/api/v1/sessions/manual", { body })) as StudySession,
...
export function useCreateActivity() {
  const invalidate = useInvalidateAll();
  return useMutation({
    mutationFn: async (body: components["schemas"]["ActivityCreate"]) =>
      unwrap(await api.POST("/api/v1/activities", { body })) as ActivityDetail,
    onSuccess: invalidate,
  });
}
...
export function useRecoveryPreview(activityId: string) {
  return useMutation({
    mutationFn: async (body: components["schemas"]["RecoveryPreviewIn"]) =>
      unwrap(
        await api.POST("/api/v1/activities/{activity_id}/recovery-plans/preview", {
...
export function useApplyRecovery(activityId: string) {
  ...
  return useMutation({
    mutationFn: async (body: components["schemas"]["RecoveryPreviewIn"]) =>
      unwrap(
        await api.POST("/api/v1/activities/{activity_id}/recovery-plans", {
          params: { path: { activity_id: activityId } },
          body,
        }),
      ) as RecoveryPlan,
...
export function useToggleTask() {
  ...
  return useMutation({
    mutationFn: async ({ id, done }: { id: string; done: boolean }) =>
      rawJson(`/api/v1/tasks/${id}/${done ? "complete" : "uncomplete"}`, { method: "POST" }),
    onSuccess: invalidate,
  });
}
