...
import { Button, EmptyState, Select, Spinner, Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui";
    ...
    <div className="flex flex-col gap-[14px] desktop:gap-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          <span className="text-[13px] text-neutral-400">Repetição espaçada</span>
          <h1 className="text-[25px] leading-[1.15] desktop:text-[32px] desktop:leading-[1.1]">Revisões</h1>
        </div>
        {acts.length > 1 ? (
          <Select value={activityId ?? ""} onChange={(e) => setParam("objetivo", e.target.value || null)} className="min-h-[36px] w-auto py-1 text-[13px]" aria-label="Filtrar por objetivo">
...
function PendingList({ activityId, today, titleOf }: { activityId?: string; today: string; titleOf: (id: string) => string | undefined }) {
    ...
    <div className="flex flex-col gap-5">
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <Group title="Hoje" kicker="kicker-accent" items={groups.today} today={today} titleOf={titleOf} empty={groups.overdue.length === 0 ? "Nada para revisar hoje." : undefined} />
...
function DoneList({ activityId, today, titleOf }: { activityId?: string; today: string; titleOf: (id: string) => string | undefined }) {
  const online = useOnline();
  const list = useRevisions({ status: "done", activity_id: activityId });
  if (list.isPending) return <Loading />;
  if (list.isError) return <LoadError online={online} onRetry={() => void list.refetch()} />;
  if (list.data.length === 0) {
    return <EmptyState title="Nenhuma revisão concluída ainda." description="Quando você concluir uma revisão, ela aparece aqui com a data." />;
  }
  return (
    <ul className="flex flex-col gap-2" aria-label="Revisões concluídas">
      {list.data.map((r) => (
        <RevisionRow key={r.id} revision={r} today={today} activityTitle={titleOf(r.activity_id)} className="bg-surface shadow-sm" />
      ))}
    </ul>
  );
}
...
function groupPending(items: Revision[], today: string) {
  const g = { overdue: [] as Revision[], today: [] as Revision[], soon: [] as Revision[], later: [] as Revision[] };
...
function Group({ title, kicker = "kicker", items, today, titleOf, empty }: { title: string; kicker?: string; items: Revision[]; today: string; titleOf: (id: string) => string | undefined; empty?: string }) {
...
function Figure({ value, label, tone }: { value: number; label: string; tone?: "pending" | "accent" }) {
  return (
    <div className="rounded-md bg-surface px-3 py-[10px]">
      <span className={cn("block text-[22px] font-medium desktop:text-[25px]", tone === "pending" && "text-pending", tone === "accent" && "text-accent")}>{value}</span>
...
function Loading() {
  return (
    <div className="flex justify-center py-12" role="status" aria-label="Carregando revisões">
...
function LoadError({ online, onRetry }: { online: boolean; onRetry: () => void }) {
  return (
    <EmptyState
      ...
      description={online ? "Tente de novo em instantes." : "Sem conexão: as revisões aparecem quando você voltar à internet."}
