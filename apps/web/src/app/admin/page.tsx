...
const SECTIONS = [
  ...
  { to: "/admin/auditoria", end: false, label: "Auditoria", icon: ClockCounterClockwise },
];
    ...
    <div className="min-h-dvh bg-canvas text-primary">
      <header className="sticky top-0 z-30 border-b border-divider bg-canvas">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-3 px-4 py-2 desktop:px-6">
          <Link to="/admin" className="flex min-h-[44px] min-w-0 items-center gap-3" aria-label="Painel administrativo — visão geral">
            <Logo size={22} />
            <span className="kicker hidden truncate xs:inline">Painel administrativo</span>
          </Link>
          <Link to="/app" className="inline-flex min-h-[44px] shrink-0 items-center gap-1.5 text-[14px] text-accent">
            <ArrowLeft size={16} aria-hidden /> Voltar ao app
          </Link>
        </div>
        <nav aria-label="Seções do painel" className="overflow-x-auto desktop:hidden">
            ...
            {SECTIONS.map((s) => (
              <li key={s.to}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn("inline-flex min-h-[44px] items-center whitespace-nowrap border-b-2 px-3 text-[14px]", isActive ? "border-accent text-accent" : "border-transparent text-neutral-400")
                  }
                >
                  {s.label}
                </NavLink>
      ...
      <div className="mx-auto flex max-w-[1440px]">
        <nav aria-label="Seções do painel" className="sticky top-[61px] hidden h-[calc(100dvh-61px)] w-[240px] shrink-0 overflow-y-auto border-r border-divider p-3 desktop:block">
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      ...
                      isActive ? "bg-accent-900 text-accent" : "text-neutral-400 hover:bg-surface hover:text-primary",
                    )
                  }
                >
        ...
        <main className="flex min-w-0 flex-1 flex-col gap-[14px] px-4 pb-12 pt-4 desktop:gap-6 desktop:px-8 desktop:pt-8">
          <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-[14px] desktop:gap-6">
            {!online ? <Banner kind="offline">Sem conexão. O painel precisa de internet: os dados podem estar desatualizados e as alterações não serão salvas.</Banner> : null}
            <Routes>
...
function OverviewPage() {
  ...
  return (
    <>
      <AdminTitle
        title="Visão geral"
        subtitle={overview.data ? `Números gerados em ${fmtDate(overview.data.generated_at)}.` : undefined}
        actions={
          <Button variant="secondary" size="lg" loading={overview.isFetching} onClick={() => void overview.refetch()}>
            Atualizar
          </Button>
        }
      />
...
function OverviewGrid({ o }: { o: AdminOverview }) {
  return (
    <div className="grid grid-cols-1 gap-3 tablet:grid-cols-2 desktop:grid-cols-3">
      <StatCard label="Usuários" value={headline(o.users)}>
        <DictLines data={o.users} skip="total" />
      </StatCard>
      <StatCard label="Assinaturas por situação">
        <StatusLines data={o.subscriptions} empty="Nenhuma assinatura." />
      </StatCard>
      <StatCard label="Objetivos" value={headline(o.activities)}>
        <DictLines data={o.activities} skip="total" />
      </StatCard>
      <StatCard label="Sessões em 7 dias" value={o.sessions_7d.toLocaleString("pt-BR")} />
      <StatCard label="Fila de envio">
        <StatusLines data={o.outbox} empty="Fila vazia." />
        <Link to="/admin/filas" className="inline-flex min-h-[44px] items-center text-[13px] text-accent">
          Ver filas
        </Link>
      </StatCard>
      <StatCard label="Importações com falha em 7 dias" value={o.imports_failed_7d.toLocaleString("pt-BR")} />
      <StatCard label="Uso de IA em 7 dias">
        <DictLines data={o.ai_usage_7d} />
      </StatCard>
      <StatCard label="Eventos de cobrança em 7 dias">
        <StatusLines data={o.billing_events_7d} empty="Nenhum evento." />
...
function headline(data: Record<string, unknown>) {
  return typeof data.total === "number" ? data.total.toLocaleString("pt-BR") : undefined;
}
    ...
    <ul className="flex flex-col gap-1.5 text-[13px]">
      {entries.map(([s, n]) => (
        <li key={s} className="flex items-center justify-between gap-3">
          ...
          <span className="tnum">{n.toLocaleString("pt-BR")}</span>
        </li>
      ))}
    </ul>
  );
