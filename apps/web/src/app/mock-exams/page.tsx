...
export default function MockExamsPage() {
  ...
  const remove = async () => {
    ...
    try {
      ...
      toast.success(tx("Simulado excluído"));
      setDeleting(null);
    } catch (e) {
      toast.error(tx("Não foi possível excluir"), errorMessage(e));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-[14px] desktop:gap-6">
      ...
      >
        <CaretLeft size={14} aria-hidden /> {actTitle ?? tx("Objetivo")}
      </Link>
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          <span className="kicker-accent">
            {tx("Simulados{{v0}}", { v0: actTitle ? ` · ${actTitle}` : "" })}
          </span>
        ...
        {mocks.data && mocks.data.count > 0 ? (
          <Button variant="primary" size="lg" onClick={() => setEditing("new")} data-tour="simulados-novo">
            <Plus size={16} aria-hidden /> {tx("Novo simulado")}
          </Button>
        ) : null}
      </header>
      ...
      {notFound ? (
        <EmptyState
          title={tx("Objetivo não encontrado.")}
          description={tx("Ele pode ter sido excluído.")}
          action={
            <Button asChild>
              <Link to="/app/objetivos">{tx("Ver objetivos")}</Link>
            </Button>
          }
        />
      ) : mocks.isPending ? (
        <div className="flex justify-center py-20" role="status" aria-label={tx("Carregando simulados")}>
          <Spinner className="h-6 w-6" />
        </div>
      ) : mocks.isError || !mocks.data ? (
        <EmptyState
          title={tx("Não foi possível carregar os simulados.")}
          description={
            online
              ? tx("Tente de novo em instantes.")
              : tx("Sem conexão: os simulados aparecem quando você voltar à internet.")
          }
          action={<Button onClick={() => void mocks.refetch()}>{tx("Tentar de novo")}</Button>}
        />
      ) : mocks.data.count === 0 ? (
      ...
      <ConfirmDialog
        ...
        title={tx("Excluir simulado?")}
        description={
          deleting
            ? tx('"{{v0}}" de {{v1}} sai do histórico e das médias por matéria. Não dá para desfazer.', {
                v0: deleting.title,
                v1: fmtDayShort(deleting.taken_on),
              })
            : undefined
        }
...
function Overview({
  ...
  data: MockOverview;
  onEdit: (e: MockExam) => void;
  onDelete: (e: MockExam) => void;
}) {
  ...
  return (
    <>
      <div className="tnum grid grid-cols-3 gap-[10px] desktop:gap-4" data-tour="simulados-numeros">
        <Stat
          label={tx("Último")}
          value={data.last_percent != null ? fmtPct(data.last_percent) : "—"}
          hint={last ? tx("{{v0}} de {{v1}}", { v0: last.correct, v1: last.total_questions }) : undefined}
        />
        <Stat
          label={tx("Melhor")}
          value={data.best_percent != null ? fmtPct(data.best_percent) : "—"}
          hint={`${data.count} ${data.count === 1 ? "simulado" : "simulados"}`}
        />
        <Stat
          label={tx("Vs. anterior")}
          value={change != null ? fmtPP(change) : "—"}
          tone={change == null || change === 0 ? undefined : change > 0 ? "success" : "error"}
          hint={
            change == null ? tx("a partir do 2º") : change > 0 ? "melhorou" : change < 0 ? "caiu" : "igual"
          }
        />
      </div>

      <Card elev="sm" className="gap-3 p-4" data-tour="simulados-evolucao">
        <span className="kicker">{tx("Evolução")}</span>
        {exams.length < 2 ? (
          <p className="text-[13px] text-neutral-400">
            {tx("A evolução aparece a partir do segundo simulado.")}
          </p>
        ) : (
      ...
      {data.subjects.length > 0 ? (
        <Card elev="sm" className="gap-3 p-4" data-tour="simulados-materias">
          <span className="kicker">{tx("Por matéria · mais fracas primeiro")}</span>
                  ...
                  <tr key={s.subject_id ?? s.subject_title} className="border-t border-divider">
                    ...
                    <td className="py-2 pr-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Bar
                          value={(s.percent ?? 0) / 100}
                          color={
                            (s.percent ?? 0) >= 70 ? "success" : (s.percent ?? 0) >= 50 ? "accent" : "pending"
                          }
                          className="flex-1"
                          label={tx("{{v0}}: {{v1}}% de acertos", {
                            v0: s.subject_title,
                            v1: s.percent ?? 0,
                          })}
                        />
        ...
        <p className="text-[13px] text-neutral-400" data-tour="simulados-materias">
          {tx("Registre os acertos por matéria para ver quais estão mais fracas.")}
        </p>
      )}
              ...
              <Card elev="sm" className="flex-row flex-wrap items-center justify-between gap-3 p-4">
                <div className="flex min-w-0 flex-col gap-0.5">
                  ...
                  <span className="text-[12px] text-neutral-400">
                    {[
                      fmtDayShort(e.taken_on),
                      tx("{{v0}} de {{v1}} questões", { v0: e.correct, v1: e.total_questions }),
                      e.duration_minutes ? tx("{{v0}} min", { v0: e.duration_minutes }) : null,
                      e.subjects.length
                        ? `${e.subjects.length} ${e.subjects.length === 1 ? tx("matéria") : tx("matérias")}`
                        : null,
                    ]
                      .filter(Boolean)
                      .join(" · ")}
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() => onEdit(e)}
                    aria-label={tx("Editar {{v0}} de {{v1}}", {
                      v0: e.title,
                      v1: fmtDayShort(e.taken_on),
                    })}
                  >
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() => onDelete(e)}
                    aria-label={tx("Excluir {{v0}} de {{v1}}", {
                      v0: e.title,
                      v1: fmtDayShort(e.taken_on),
                    })}
                  >
...
function Stat({
  ...
  return (
    <Card elev="sm" className="gap-0.5 p-3 desktop:p-4">
      ...
      <span
        className={cn(
          "text-[20px] font-medium desktop:text-[25px]",
...
function fmtPP(v: number): string {
  const abs = Math.abs(v).toLocaleString(intlLocale, { maximumFractionDigits: 1 });
  return tx("{{v0}}{{v1}} p.p.", { v0: v > 0 ? "+" : v < 0 ? "−" : "", v1: abs });
}
...
function EvolutionChart({ exams }: { exams: MockExam[] }) {
  ...
  const summary = tx("Acertos por simulado, do mais antigo ao mais recente: {{v0}}.", {
    v0: pts.map((p) => fmtPct(Math.round(p.pct * 10) / 10)).join(", "),
  });
...
function rowsFrom(exam: MockExam | null): RowState[] {
  if (!exam || exam.subjects.length === 0)
    return [{ key: 1, subjectId: "", title: "", total: "", correct: "" }];
...
const toInt = (v: string): number | null =>
  v.trim() === "" || !/^\d+$/.test(v.trim()) ? null : Number(v.trim());
...
function ExamDialog({
  ...
  const [title, setTitle] = React.useState(exam?.title ?? tx("Simulado"));
  ...
  const [total, setTotal] = React.useState(
    exam && exam.subjects.length === 0 ? String(exam.total_questions) : "",
  );
  const [correct, setCorrect] = React.useState(
    exam && exam.subjects.length === 0 ? String(exam.correct) : "",
  );
  ...
  const addRow = () =>
    setRows((rs) => [
      ...rs,
      { key: Math.max(0, ...rs.map((r) => r.key)) + 1, subjectId: "", title: "", total: "", correct: "" },
    ]);
  const removeRow = (key: number) => setRows((rs) => (rs.length > 1 ? rs.filter((r) => r.key !== key) : rs));

  const rowTotals = rows.reduce(
    (acc, r) => ({
      total: acc.total + (toInt(r.total) ?? 0),
      correct: acc.correct + (toInt(r.correct) ?? 0),
    }),
    { total: 0, correct: 0 },
  );
  ...
  const validate = (): Record<string, string> => {
    ...
    if (!title.trim()) e.title = tx("Dê um nome ao simulado.");
    if (!takenOn) e.takenOn = tx("Informe a data.");
    if (mode === "total") {
      const t = toInt(total);
      const c = toInt(correct);
      if (t == null || t < 1) e.total = tx("Informe o total de questões.");
      else if (t > 5000) e.total = tx("No máximo 5000 questões.");
      if (c == null) e.correct = tx("Informe os acertos (pode ser 0).");
      else if (t != null && c > t) e.correct = tx("Os acertos não podem passar do total.");
    } else {
      rows.forEach((r) => {
        const t = toInt(r.total);
        const c = toInt(r.correct);
        if (!r.subjectId) e[`s${r.key}`] = tx("Escolha a matéria.");
        else if (r.subjectId === OTHER && !r.title.trim()) e[`s${r.key}`] = tx("Informe o nome da matéria.");
        if (t == null || t < 1) e[`t${r.key}`] = tx("Total?");
        else if (t > 1000) e[`t${r.key}`] = tx("Máx. 1000");
        if (c == null) e[`c${r.key}`] = tx("Acertos?");
        else if (t != null && c > t) e[`c${r.key}`] = tx("Maior que o total");
      });
    }
    const d = duration.trim();
    if (d && (toInt(d) == null || toInt(d)! < 1 || toInt(d)! > 1440))
      e.duration = tx("Use minutos entre 1 e 1440.");
    return e;
  };
  ...
  const submit = async (ev: React.FormEvent) => {
    ...
    if (mode === "total") {
      ...
      body.subjects = rows.map((r) =>
        r.subjectId === OTHER
          ? { subject_title: r.title.trim(), total: toInt(r.total)!, correct: toInt(r.correct)! }
          : { subject_id: r.subjectId, total: toInt(r.total)!, correct: toInt(r.correct)! },
      );
    }
    try {
      await save.mutateAsync({ id: exam?.id, body });
      toast.success(exam ? tx("Simulado atualizado") : tx("Simulado registrado"));
      onClose();
    } catch (err) {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent
        mode="sheet"
        title={exam ? tx("Editar simulado") : tx("Registrar simulado")}
        className="tablet:w-[min(600px,calc(100%-32px))]"
      >
        <form onSubmit={(e) => void submit(e)} className="flex flex-col gap-[14px]" noValidate>
          <div className="grid gap-3 tablet:grid-cols-[minmax(0,1fr)_170px]">
            <Field label={tx("Nome")} htmlFor={`${uid}-title`} error={errors.title}>
              <Input
                id={`${uid}-title`}
                value={title}
            ...
            <Field label={tx("Data")} htmlFor={`${uid}-date`} error={errors.takenOn}>
              <Input
                id={`${uid}-date`}
                type="date"
                max={todayIso()}
                ...
                invalid={!!errors.takenOn}
              />
            </Field>
          </div>

          <div className="flex flex-col gap-2">
            <span className="text-[12px] text-[color-mix(in_srgb,var(--color-text-primary)_70%,transparent)]">
              {tx("Resultado")}
            </span>
            <Seg<Mode>
              label={tx("Como informar o resultado")}
              value={mode}
              onChange={setMode}
              block
              options={[
                { value: "total", label: tx("Total e acertos") },
                { value: "materias", label: tx("Por matéria") },
              ]}
            />
          </div>
          ...
          {mode === "total" ? (
            <div className="grid grid-cols-2 gap-3">
              <Field label={tx("Total de questões")} htmlFor={`${uid}-total`} error={errors.total}>
                <Input
                  id={`${uid}-total`}
                  type="number"
                  inputMode="numeric"
              ...
              <Field label={tx("Acertos")} htmlFor={`${uid}-correct`} error={errors.correct}>
                <Input
                  id={`${uid}-correct`}
                  type="number"
                  inputMode="numeric"
                  min={0}
                  max={toInt(total) ?? undefined}
                  value={correct}
            ...
            <fieldset className="flex flex-col gap-3">
              <legend className="sr-only">{tx("Resultado por matéria")}</legend>
              {rows.map((r, i) => (
                <div key={r.key} className="flex flex-col gap-2 rounded-md bg-canvas p-3">
                  <div className="flex items-end gap-2">
                    <Field
                      label={tx("Matéria {{v0}}", { v0: i + 1 })}
                      htmlFor={`${uid}-s${r.key}`}
                      error={errors[`s${r.key}`]}
                      className="min-w-0 flex-1"
                    >
                      <Select
                        id={`${uid}-s${r.key}`}
                        value={r.subjectId}
                        onChange={(e) => setRow(r.key, { subjectId: e.target.value })}
                        invalid={!!errors[`s${r.key}`]}
                      >
                        ...
                        <option value={OTHER}>{tx("Outra matéria (digitar)")}</option>
                      </Select>
                    </Field>
                    <Button
                      type="button"
                      ...
                      aria-label={tx("Remover matéria {{v0}}", { v0: i + 1 })}
                    >
                      <X size={16} aria-hidden />
                    </Button>
                  </div>
                  {r.subjectId === OTHER ? (
                    <Field label={tx("Nome da matéria")} htmlFor={`${uid}-n${r.key}`}>
                      <Input
                        id={`${uid}-n${r.key}`}
                        value={r.title}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Field label={tx("Questões")} htmlFor={`${uid}-t${r.key}`} error={errors[`t${r.key}`]}>
                      <Input
                        id={`${uid}-t${r.key}`}
                        type="number"
                        inputMode="numeric"
                        ...
                        max={1000}
                        value={r.total}
                        onChange={(e) => setRow(r.key, { total: e.target.value })}
                        invalid={!!errors[`t${r.key}`]}
                      />
                    </Field>
                    <Field label={tx("Acertos")} htmlFor={`${uid}-c${r.key}`} error={errors[`c${r.key}`]}>
                      <Input
                        id={`${uid}-c${r.key}`}
                        type="number"
                        inputMode="numeric"
                        min={0}
                        max={toInt(r.total) ?? undefined}
                        value={r.correct}
                        onChange={(e) => setRow(r.key, { correct: e.target.value })}
                        invalid={!!errors[`c${r.key}`]}
                      />
                    </Field>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <Button type="button" variant="ghost" size="sm" onClick={addRow}>
                  <Plus size={14} aria-hidden /> {tx("Adicionar matéria")}
                </Button>
                <span className="tnum text-[13px] text-neutral-400" aria-live="polite">
                  {tx("Total: {{v0}} de {{v1}}", { v0: rowTotals.correct, v1: rowTotals.total })}
                  {rowTotals.total > 0 && rowTotals.correct <= rowTotals.total
                    ? ` · ${fmtPct(Math.round((1000 * rowTotals.correct) / rowTotals.total) / 10)}`
                    : ""}
                </span>
              </div>
            </fieldset>
          ...
          <Field label={tx("Observações (opcional)")} htmlFor={`${uid}-notes`}>
            <Textarea
              id={`${uid}-notes`}
              value={notes}
          ...
          <DialogActions>
            ...
            <Button type="submit" variant="primary" size="lg" loading={save.isPending}>
              {exam ? tx("Salvar alterações") : tx("Registrar")}
            </Button>
          </DialogActions>
        </form>
      </DialogContent>
