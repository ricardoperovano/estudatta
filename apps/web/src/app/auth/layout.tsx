  ...
  return (
    <div className="app-backdrop flex min-h-dvh flex-col">
      <div className="mx-auto flex w-full max-w-[1040px] flex-1 flex-col px-4 pb-6 pt-[max(20px,calc(12px+env(safe-area-inset-top,0px)))] desktop:flex-row desktop:items-center desktop:gap-12 desktop:px-8">
        ...
        <aside className="hidden flex-1 flex-col gap-6 desktop:flex" aria-label={t("Sobre o Estudatta")}>
          <a
            href={siteLink("/")}
            className="flex w-fit items-center gap-2.5 text-primary no-underline"
            aria-label={t("Estudatta — site")}
          >
            <Symbol size={32} />
          ...
          <h2 className="max-w-[16ch] text-[36px] font-medium leading-[1.1] tracking-[-0.02em]">
            {t("Saiba o que fazer hoje. Retome quando atrasar.")}
          </h2>
          <ul className="m-0 flex list-none flex-col gap-3 p-0 text-[15px] text-neutral-300">
            <Benefit
              icon={<SunHorizon size={20} weight="duotone" />}
              text={t("Um plano do dia que cabe na sua rotina, com meta de tempo e dias da semana.")}
            />
            <Benefit
              icon={<ArrowsClockwise size={20} weight="duotone" />}
              text={t("Ficou para trás? O tempo vira pendência e é recuperado aos poucos, sem culpa.")}
            />
            <Benefit
              icon={<Sparkle size={20} weight="duotone" />}
              text={t("Revisões espaçadas, simulados, conquistas e o Tatá te acompanhando nas sessões.")}
            />
          </ul>
        </aside>
        ...
        <main className="mx-auto flex w-full max-w-[440px] flex-1 flex-col gap-4 desktop:max-w-[420px]">
          <a
            href={siteLink("/")}
            aria-label={t("Estudatta — site")}
            className="flex w-fit items-center gap-2 text-primary no-underline desktop:hidden"
          >
            <Symbol size={28} />
          ...
          <div className="mt-2 flex items-end gap-3">
            <TataSvg
              mood={mood}
              size={84}
              title={t("Tatá, o mascote do Estudatta")}
            ...
            <p className="tata-bubble tata-bubble--left m-0 mb-4 min-w-0 flex-1 rounded-lg border border-divider bg-surface px-3 py-2 text-[13px] leading-[1.35] text-primary shadow-sm">
              {greeting ?? t("Oi! Que bom te ver. Seu plano de hoje está esperando.")}
            </p>
          </div>
          <section
            className="hero-soft rounded-[22px] border border-divider p-5 shadow-sm desktop:p-6"
          ...
          <p className="mt-auto px-1 pt-4 text-center text-[12px] text-neutral-500">
            {t("Sem promessa de aprovação ou fluência: o Estudatta organiza o estudo que você já faz.")}
          </p>
        </main>
      </div>
    </div>
...
function Benefit({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <li className="flex items-start gap-3">
      <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent-900 text-accent">
        {icon}
      </span>
      <span>{text}</span>
    </li>
