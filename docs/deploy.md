...
## Requisitos
- Linux com Docker Engine + Compose v2, Nginx (ou Caddy) para TLS, domínio apontando para o servidor (`estudatta.com.br`, `www`).
...
6. TLS: instale `infra/nginx/estudatta.conf.example` em `/etc/nginx/sites-available/estudatta`, ajuste o domínio, `certbot --nginx -d estudatta.com.br -d www.estudatta.com.br`, `nginx -t && systemctl reload nginx`.
7. Verifique `https://estudatta.com.br/api/v1/health/ready` (deve responder `{"status":"ok"}`) e a instalação do PWA no celular (push exige HTTPS e, no iOS, o app na tela inicial).
...
- Rollback de código: `git checkout <tag-anterior>` + `build` + `up -d`. Rollback de banco: `alembic downgrade <revisão>` antes de voltar o código, ou restauração de backup (abaixo). Faça `make backup` antes de cada atualização.
