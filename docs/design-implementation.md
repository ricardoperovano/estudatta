...
| `06 Landing.dc.html` | Site público | Projeto separado `estudatta-site` (HTML/CSS/JS puros, `assets/css/site.css` com os mesmos tokens) |
| `07 Divulgacao.dc.html` + `divulgacao/` | Peças e mensagens de campanha | Capa de compartilhamento e mensagens de campanha usadas no site (`estudatta-site`) |
...
- (Site, agora em `estudatta-site`) Na landing, o CTA principal virou "Começar grátis"/"Criar conta" porque o cadastro já existe; a lista de e-mail ("Receber novidades") permanece como opção secundária (o mockup previa "Quero acesso inicial" por ser pré-lançamento).
